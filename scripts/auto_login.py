...
TWO_FACTOR_WAIT = int(os.environ.get(
    "TWO_FACTOR_WAIT", "120"))  # 2FA验证 默认等 120 秒
...
class Hysteria2Proxy:
    ...
    def parse_url(self):
        ...
        try:
            ...
            print(
                f"  🔐 认证: {password[:4]}...{password[-4:] if len(password) > 8 else '***'}")
    ...
    def get_drissionpage_proxy(self):
        """获取 DrissionPage 代理配置"""
        if not self.enabled:
            return None
...
class AutoLogin:
    ...
    def shot(self, page, name):
        ...
        try:
            page.get_screenshot(path=f, full_page=True)
            self.shots.append(f)
        except:
            pass
        return f
    ...
    def get_session(self, page):
        """提取 Session Cookie"""
        try:
            for c in page.cookies(all_info=True):
...
等待时间：{TWO_FACTOR_WAIT} 秒""")
    ...
    def login_github(self, page):
        ...
        try:
            page.ele('input[type="submit"], button[type="submit"]').click()
        except:
            pass
        ...
        # 错误
        try:
            err = page.ele('.flash-error', timeout=2)
            if err:
...
<b>时间:</b> {time.strftime('%Y-%m-%d %H:%M:%S')}"""
    ...
    def run(self):
        ...
        try:
            ...
            # 如果有代理
            if self.proxy.enabled:
                proxy_config = self.proxy.get_drissionpage_proxy()
                if proxy_config:
            ...
            try:
                ...
                page.get(SIGNIN_URL, timeout=60)
                page.wait.load_start()
                time.sleep(2)
                ...
                # 5. 验证
                self.log("步骤5: 保活", "STEP")
                self.keepalive(page)

                # 6. 提取并保存新 Cookie
                self.log("步骤6: 保存 Cookie", "STEP")
                new = self.get_session(page)
                if new and new != self.gh_session:
                    self.save_cookie(new)
